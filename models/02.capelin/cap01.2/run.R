...
## i <- which(sapply(lkh,function(x){x$name}) == "siQ1.cap")
## lkh[[i]]$weight <- lkh[[i]]$weight * 100
## i <- which(sapply(lkh,function(x){x$name}) == "siQ3.cap")
## lkh[[i]]$weight <- lkh[[i]]$weight * 100
...
figName <- "adist_stk_cap.ps"
  ## plot(fit, data='stock.std')# + facet_wrap(~stock) +
  ## ggRunName() + ggRunNameSize(6) +
  ## ggsave(paste(dirFigs,figName, sep="/"), device="ps")
ggAgeDistStk2(fit, stkName="cap", ageVec=1:12, plusGroup=12) +
  labs(subtitle=figName) +
  ggRunName() + ggRunNameSize(6)
ggsave(paste(dirFigs,figName, sep="/"), device="ps")
...
##   ggRunName() + ggRunNameSize(6) +
##   ggsave(paste(dirFigs,figName, sep="/"), device="ps")
ggplot(fit$suitability %>% filter(stock %in% c("cap") & year == 80 &
...
figName <- "ldist_cap_com.ps"
  tmp$ldist.cap.com +
  labs(subtitle=figName) +
  ggRunName() + ggRunNameSize(6)
  ggsave(paste(dirFigs,figName, sep="/"), device="ps")

figName <- "ldist_cap_survQ1.ps"
  tmp$ldist.cap.surQ1 +
  labs(subtitle=figName) +
  ggRunName() + ggRunNameSize(6)
  ggsave(paste(dirFigs,figName, sep="/"), device="ps")

figName <- "ldist_cap_survQ3.ps"
  tmp$ldist.cap.surQ3 +
  labs(subtitle=figName) +
  ggRunName() + ggRunNameSize(6)
ggsave(paste(dirFigs,figName, sep="/"), device="ps")

figName <- "alk_cap_survQ1.ps"
  tmp$alk.cap.surQ1 +
  labs(subtitle=figName) +
  ggRunName() + ggRunNameSize(6)
ggsave(paste(dirFigs,figName, sep="/"), device="ps")

## figName <- "alk_cap_survQ3.ps"
##   tmp$alk.cap.surQ3 +
##   labs(subtitle=figName) +
##   ggRunName() + ggRunNameSize(6)
## ggsave(paste(dirFigs,figName, sep="/"), device="ps")

## figName <- "adist_cap_com.ps"
##   tmp$adist.cap.com +
##   labs(subtitle=figName) +
##   ggRunName() + ggRunNameSize(6)
## ggsave(paste(dirFigs,figName, sep="/"), device="ps")

## figName <- "adist_cap_survQ1.ps"
##   tmp$adist.cap.surQ1 +
##   labs(subtitle=figName) +
##   ggRunName() + ggRunNameSize(6)
## ggsave(paste(dirFigs,figName, sep="/"), device="ps")

## figName <- "adist_cap_survQ3.ps"
##   tmp$adist.cap.surQ3 +
##   labs(subtitle=figName) +
##   ggRunName() + ggRunNameSize(6)
## ggsave(paste(dirFigs,figName, sep="/"), device="ps")
...
figName <- "bubble_cap_ldist.ps"
  bubbles$ldist +
  labs(subtitle=figName) +
  ggRunName() + ggRunNameSize(6)
ggsave(paste(dirFigs,figName, sep="/"), device="ps")
  
figName <- "bubble_cap_aldist.ps"
  bubbles$aldist +
  labs(subtitle=figName) +
  ggRunName() + ggRunNameSize(6)
ggsave(paste(dirFigs,figName, sep="/"), device="ps")
...
figName <- "growth_cap_survQ1.ps"
  grplot$alk.cap.surQ1 +
  labs(subtitle=figName) +
  ggRunName() + ggRunNameSize(6)
ggsave(paste(dirFigs,figName, sep="/"), device="ps")

figName <- "growth_cap_survQ3.ps"
  grplot$alk.cap.surQ3 +
  labs(subtitle=figName) +
  ggRunName() + ggRunNameSize(6)
ggsave(paste(dirFigs,figName, sep="/"), device="ps")
...
load("../../github/models/02.capelin/cap03/WGTS/WGTS.Rdata")
fit1 <- out
## load("../cap01_xx/WGTS2/WGTS.Rdata")
## fit2 <- out
load("../cap01.2/WGTS/WGTS.Rdata")
fit3 <- out

fitL <- bind.gadget.fit("fit1"=fit1, "fit2"=fit2, "fit3"=fit3)

# SSB
tmp <- fitL$res.by.year %>% filter(stock=="cap") ## %>%
...
figName <- "ldist_cap_com_comp"
ggLenDist(Gfit=fitL, compName="ldist.cap.com", yrs=50:60)##  +
    ## theme(strip.background=element_blank(),
    ##       strip.text.x = element_blank())
    ## ggsave(paste(dirFigs,figName, sep="/"), device="ps")
ggLenDist(Gfit=fitL, compName="ldist.cap.surQ1", yrs=50:60)
ggLenDist(Gfit=fitL, compName="ldist.cap.surQ3", yrs=50:60)
ggAgeDist(Gfit=fitL, compName="adist.cap.com", yrs=50:60)
ggAgeDist(Gfit=fitL, compName="adist.cap.surQ1", yrs=50:60)
ggAgeDist(Gfit=fitL, compName="adist.cap.surQ3", yrs=50:60)

# selection pattern
tmp <- fitL$suitability %>% filter(stock %in% c("cap") & year == 80 &
                                  step %in% ifelse(substring(fleet,1,6)=="survQ1",1,3))
...
ggplot(tmp, aes(model, ldist.cap.com)) +
    geom_bar(stat="identity")
