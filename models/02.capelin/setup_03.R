...
source('setup_model_stk_age_0.R')
...
read.gadget.parameters(sprintf('%s/params.out',gd$dir)) %>% 
  ...
  init_guess(paste0(stock_names,'.recl'), grw.constants["recl"], grw.constants["recl"]*0.2, grw.constants["recl"]*2, 1) %>%
  ## init_guess(paste0(stock_names,'.recl'), min(defaults$length)*2, min(defaults$length), min(defaults$length)*3,1) %>%
  init_guess(paste0(stock_names,'.rec.sd'), init.sigma$stddev[1]*0.9, init.sigma$stddev[1]*0.2, init.sigma$stddev[1]*2,1) %>%
  init_guess(paste0(stock_names,'.Linf'), grw.constants["Linf"], grw.constants["Linf"]*0.8, grw.constants["Linf"]*1.2,1) %>%
  ...
  init_guess(paste0(stock_names,'.surQ3.alpha'), 0.9,  0.1, 5, 1) %>% 
  init_guess(paste0(stock_names,'.surQ3.l50'), mean(as.numeric(substring(ldist.survQ3$length,4,6))), 3, max(as.numeric(substring(ldist.survQ3$length,4,6))), 1) %>%
...
file.copy(sprintf('%s/plots.R','./'),gd$dir)
