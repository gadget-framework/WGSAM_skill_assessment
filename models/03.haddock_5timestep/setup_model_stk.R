...
tmp <- mfdb_sample_count(mdb, c('age','length','species'), list(
       ...
       timestep      = c(3,8), # month of the survey
       year          = defaults$year,
       sampling_type = 'RES',
...
grw.constants <- c(grw.constants, grw.constants[1]*(1-exp(-grw.constants[2] * (((mean(defaults$timestep[sppListi$recStep][[1]])-0.5)/12-1/12/2)-grw.constants[3]))))
...
##     geom_vline(xintercept = ((mean(defaults$timestep[sppListi$recStep][[1]])-0.5)/12-1/12/2), color = 3) +
##     geom_hline(yintercept = grw.constants["recl"], color = 3) +
##     facet_wrap(~year)

## maxlengthgroupgrowth as x2 the mean length growth from recl
## dL = (Linf-Li)*(1-exp(-Kdt)); 2*dL/lenGrp
nL.max <- ceiling(3*(grw.constants["Linf"]-grw.constants["recl"]) * (1-exp(-grw.constants["k"]*0.25))/(defaults$length[2] - defaults$length[1]))
...
stk <-
  ...
  gadget_update('initialconditions',
                normalcond = data_frame(age = as.numeric(substring(init.num$age,4,5)),
                                        ...
                                        mean = parse(text=sprintf(paste0(species_name,'.Linf*(1-exp(-1*(0.01*',species_name,'.k)*(%1$s-(',((mean(defaults$timestep[sppListi$recStep][[1]])-0.5)/12-1/12/2),'+log(1-',species_name,'.recl/',species_name,'.Linf)/(0.01*',species_name,'.k)))))'),age)) %>% # notice [... -(0.5+log ...] so length scaled considering that rec is in timestep 3
