...
sppLookup <- data.frame(nobaSpp = c("BWH","CAP","GRH","HAD","LRD","MAC","NCO","PCO","RED","SAI","SSH"),
                        ...
                        recStep = c(2,5,2,2,2,2,2,4,2,5,2), # predefined timestep of recruitment (5 subannual timesteps)
...
ageClsM3 <- ageClsListM3[[spp]]
ageClsM8 <- ageClsListM8[[spp]]
...
read.gadget.parameters(sprintf('%s/params.out',gd$dir)) %>% 
  ...
  init_guess(paste0(stock_names,'.recl'), grw.constants["recl"], grw.constants["recl"]*0.5, grw.constants["recl"]*1.3, 1) %>%
  ...
  init_guess(paste0(stock_names,'.Linf'), grw.constants["Linf"], grw.constants["Linf"]*0.8, grw.constants["Linf"]*1.4,1) %>%
  ...
  init_guess(paste0(stock_names,'.surQ1.alpha'), 0.9,  0.1, 3, 1) %>% 
  ...
  init_guess(paste0(stock_names,'.surQ3.alpha'), 0.9,  0.1, 3, 1) %>% 
